...
import static com.github.teruteru128.study.MyRandom.nextDouble;

import com.github.teruteru128.study.Factory;
import com.github.teruteru128.study.MyRandom;
...
public class MyRandomSample {
  ...
  static void random3() {
    var randomGeneratorFactory = RandomGeneratorFactory.all()
        .filter(rgf -> !rgf.name().equals("SecureRandom")).filter(rgf -> rgf.stateBits() >= 144)
        .min(Comparator.comparingInt(RandomGeneratorFactory<RandomGenerator>::stateBits))
        .orElse(RandomGeneratorFactory.of("Random"));
    System.out.println(
        randomGeneratorFactory.name() + " in " + randomGeneratorFactory.group() + " was selected");
   ...
   */
  public static void doubleSample(RandomGenerator random) {
    long start = System.nanoTime();
    for (int i = 0; i < 100000000; i++) {
      MyRandom.nextDouble(random);
    }
    long finish = System.nanoTime();
    System.out.printf("%f seconds%n", (finish - start) / 1e9);
    System.out.println(MyRandom.nextDouble(random));
  }
}
