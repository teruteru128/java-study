// https://github.com/MiniblockchainProject/CudaMiner/blob/master/cuda_ripemd160.cu
...
__global void ripemd_gpu_hash_242(int threads, uint64_t startNounce,
                                    uint32_t *g_block, uint64_t *g_hash) {
  /*
...
__kernel void ripemd_cpu_hash_242(int thr_id, int threads, uint64_t startNounce,
                                  uint32_t *d_block, uint64_t *d_hash) {

  // Alignment mit mixtab Gr\F6sse. NICHT \C4NDERN
  ...
  ripemd_gpu_hash_242<<<grid, block, shared_size>>>(threads, startNounce,
                                                    d_block, d_hash);
...
void ripemd_scanhash(int throughput, uint64_t startNonce, CBlockHeader *hdr,
                     uint64_t *d_hash, ctx *pctx) {
  char block[192];
  uint32_t r[5];
  ...
  gpuErrchk(cudaMemcpyAsync(pctx->ripemd_dblock, block, sizeof(block),
                            cudaMemcpyHostToDevice, 0));

  ripemd_cpu_hash_242(pctx->thr_id, throughput, startNonce, pctx->ripemd_dblock,
                      d_hash);
}

void ripemd160_update(const char *buf) { return; }
