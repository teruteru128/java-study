...
public class PrimeSearchTask2 implements Callable<Optional<Integer>> {
  ...
  private static final ThreadLocal<MemorySegment> threadCandidates = ThreadLocal.withInitial(() -> {
    var candidate = auto.allocate(__mpz_struct.layout()).reinterpret(auto, x0 -> {
      logger.trace("Candidate have been released: {}", format.toHexDigits(x0.address()));
      mpz_clear(x0);
    });
    mpz_init2(candidate, 1048576);
    logger.trace("candidate storage have been initialized: {}", format.toHexDigits(candidate.address()));
    return candidate;
  });
