// Generated by jextract

package com.twitter.teruteru128.preview.windows;

import java.lang.invoke.MethodHandle;
import java.lang.invoke.VarHandle;
import java.nio.ByteOrder;
import java.lang.foreign.*;
import static java.lang.foreign.ValueLayout.*;
/**
 * {@snippet :
 * struct _CSV_CONTROL_PARAM {
 *     CSV_CONTROL_OP Operation;
 *     LONGLONG Unused;
 * };
 * }
 */
public class _CSV_CONTROL_PARAM {

    public static MemoryLayout $LAYOUT() {
        return constants$2681.const$1;
    }
    public static VarHandle Operation$VH() {
        return constants$2681.const$2;
    }
    /**
     * Getter for field:
     * {@snippet :
     * CSV_CONTROL_OP Operation;
     * }
     */
    public static int Operation$get(MemorySegment seg) {
        return (int)constants$2681.const$2.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * CSV_CONTROL_OP Operation;
     * }
     */
    public static void Operation$set(MemorySegment seg, int x) {
        constants$2681.const$2.set(seg, x);
    }
    public static int Operation$get(MemorySegment seg, long index) {
        return (int)constants$2681.const$2.get(seg.asSlice(index*sizeof()));
    }
    public static void Operation$set(MemorySegment seg, long index, int x) {
        constants$2681.const$2.set(seg.asSlice(index*sizeof()), x);
    }
    public static VarHandle Unused$VH() {
        return constants$2681.const$3;
    }
    /**
     * Getter for field:
     * {@snippet :
     * LONGLONG Unused;
     * }
     */
    public static long Unused$get(MemorySegment seg) {
        return (long)constants$2681.const$3.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * LONGLONG Unused;
     * }
     */
    public static void Unused$set(MemorySegment seg, long x) {
        constants$2681.const$3.set(seg, x);
    }
    public static long Unused$get(MemorySegment seg, long index) {
        return (long)constants$2681.const$3.get(seg.asSlice(index*sizeof()));
    }
    public static void Unused$set(MemorySegment seg, long index, long x) {
        constants$2681.const$3.set(seg.asSlice(index*sizeof()), x);
    }
    public static long sizeof() { return $LAYOUT().byteSize(); }
    public static MemorySegment allocate(SegmentAllocator allocator) { return allocator.allocate($LAYOUT()); }
    public static MemorySegment allocateArray(long len, SegmentAllocator allocator) {
        return allocator.allocate(MemoryLayout.sequenceLayout(len, $LAYOUT()));
    }
    public static MemorySegment ofAddress(MemorySegment addr, Arena arena) { return RuntimeHelper.asArray(addr, $LAYOUT(), 1, arena); }
}


