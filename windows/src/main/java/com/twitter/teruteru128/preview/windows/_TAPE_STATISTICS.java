// Generated by jextract

package com.twitter.teruteru128.preview.windows;

import java.lang.invoke.MethodHandle;
import java.lang.invoke.VarHandle;
import java.nio.ByteOrder;
import java.lang.foreign.*;
import static java.lang.foreign.ValueLayout.*;
/**
 * {@snippet :
 * struct _TAPE_STATISTICS {
 *     DWORD Version;
 *     DWORD Flags;
 *     LARGE_INTEGER RecoveredWrites;
 *     LARGE_INTEGER UnrecoveredWrites;
 *     LARGE_INTEGER RecoveredReads;
 *     LARGE_INTEGER UnrecoveredReads;
 *     BYTE CompressionRatioReads;
 *     BYTE CompressionRatioWrites;
 * };
 * }
 */
public class _TAPE_STATISTICS {

    public static MemoryLayout $LAYOUT() {
        return constants$2377.const$3;
    }
    public static VarHandle Version$VH() {
        return constants$2377.const$4;
    }
    /**
     * Getter for field:
     * {@snippet :
     * DWORD Version;
     * }
     */
    public static int Version$get(MemorySegment seg) {
        return (int)constants$2377.const$4.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * DWORD Version;
     * }
     */
    public static void Version$set(MemorySegment seg, int x) {
        constants$2377.const$4.set(seg, x);
    }
    public static int Version$get(MemorySegment seg, long index) {
        return (int)constants$2377.const$4.get(seg.asSlice(index*sizeof()));
    }
    public static void Version$set(MemorySegment seg, long index, int x) {
        constants$2377.const$4.set(seg.asSlice(index*sizeof()), x);
    }
    public static VarHandle Flags$VH() {
        return constants$2377.const$5;
    }
    /**
     * Getter for field:
     * {@snippet :
     * DWORD Flags;
     * }
     */
    public static int Flags$get(MemorySegment seg) {
        return (int)constants$2377.const$5.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * DWORD Flags;
     * }
     */
    public static void Flags$set(MemorySegment seg, int x) {
        constants$2377.const$5.set(seg, x);
    }
    public static int Flags$get(MemorySegment seg, long index) {
        return (int)constants$2377.const$5.get(seg.asSlice(index*sizeof()));
    }
    public static void Flags$set(MemorySegment seg, long index, int x) {
        constants$2377.const$5.set(seg.asSlice(index*sizeof()), x);
    }
    public static MemorySegment RecoveredWrites$slice(MemorySegment seg) {
        return seg.asSlice(8, 8);
    }
    public static MemorySegment UnrecoveredWrites$slice(MemorySegment seg) {
        return seg.asSlice(16, 8);
    }
    public static MemorySegment RecoveredReads$slice(MemorySegment seg) {
        return seg.asSlice(24, 8);
    }
    public static MemorySegment UnrecoveredReads$slice(MemorySegment seg) {
        return seg.asSlice(32, 8);
    }
    public static VarHandle CompressionRatioReads$VH() {
        return constants$2378.const$0;
    }
    /**
     * Getter for field:
     * {@snippet :
     * BYTE CompressionRatioReads;
     * }
     */
    public static byte CompressionRatioReads$get(MemorySegment seg) {
        return (byte)constants$2378.const$0.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * BYTE CompressionRatioReads;
     * }
     */
    public static void CompressionRatioReads$set(MemorySegment seg, byte x) {
        constants$2378.const$0.set(seg, x);
    }
    public static byte CompressionRatioReads$get(MemorySegment seg, long index) {
        return (byte)constants$2378.const$0.get(seg.asSlice(index*sizeof()));
    }
    public static void CompressionRatioReads$set(MemorySegment seg, long index, byte x) {
        constants$2378.const$0.set(seg.asSlice(index*sizeof()), x);
    }
    public static VarHandle CompressionRatioWrites$VH() {
        return constants$2378.const$1;
    }
    /**
     * Getter for field:
     * {@snippet :
     * BYTE CompressionRatioWrites;
     * }
     */
    public static byte CompressionRatioWrites$get(MemorySegment seg) {
        return (byte)constants$2378.const$1.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * BYTE CompressionRatioWrites;
     * }
     */
    public static void CompressionRatioWrites$set(MemorySegment seg, byte x) {
        constants$2378.const$1.set(seg, x);
    }
    public static byte CompressionRatioWrites$get(MemorySegment seg, long index) {
        return (byte)constants$2378.const$1.get(seg.asSlice(index*sizeof()));
    }
    public static void CompressionRatioWrites$set(MemorySegment seg, long index, byte x) {
        constants$2378.const$1.set(seg.asSlice(index*sizeof()), x);
    }
    public static long sizeof() { return $LAYOUT().byteSize(); }
    public static MemorySegment allocate(SegmentAllocator allocator) { return allocator.allocate($LAYOUT()); }
    public static MemorySegment allocateArray(long len, SegmentAllocator allocator) {
        return allocator.allocate(MemoryLayout.sequenceLayout(len, $LAYOUT()));
    }
    public static MemorySegment ofAddress(MemorySegment addr, Arena arena) { return RuntimeHelper.asArray(addr, $LAYOUT(), 1, arena); }
}


