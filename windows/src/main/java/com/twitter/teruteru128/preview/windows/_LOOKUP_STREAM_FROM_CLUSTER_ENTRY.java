// Generated by jextract

package com.twitter.teruteru128.preview.windows;

import java.lang.invoke.MethodHandle;
import java.lang.invoke.VarHandle;
import java.nio.ByteOrder;
import java.lang.foreign.*;
import static java.lang.foreign.ValueLayout.*;
/**
 * {@snippet :
 * struct _LOOKUP_STREAM_FROM_CLUSTER_ENTRY {
 *     DWORD OffsetToNext;
 *     DWORD Flags;
 *     LARGE_INTEGER Reserved;
 *     LARGE_INTEGER Cluster;
 *     WCHAR FileName[1];
 * };
 * }
 */
public class _LOOKUP_STREAM_FROM_CLUSTER_ENTRY {

    public static MemoryLayout $LAYOUT() {
        return constants$2678.const$4;
    }
    public static VarHandle OffsetToNext$VH() {
        return constants$2678.const$5;
    }
    /**
     * Getter for field:
     * {@snippet :
     * DWORD OffsetToNext;
     * }
     */
    public static int OffsetToNext$get(MemorySegment seg) {
        return (int)constants$2678.const$5.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * DWORD OffsetToNext;
     * }
     */
    public static void OffsetToNext$set(MemorySegment seg, int x) {
        constants$2678.const$5.set(seg, x);
    }
    public static int OffsetToNext$get(MemorySegment seg, long index) {
        return (int)constants$2678.const$5.get(seg.asSlice(index*sizeof()));
    }
    public static void OffsetToNext$set(MemorySegment seg, long index, int x) {
        constants$2678.const$5.set(seg.asSlice(index*sizeof()), x);
    }
    public static VarHandle Flags$VH() {
        return constants$2679.const$0;
    }
    /**
     * Getter for field:
     * {@snippet :
     * DWORD Flags;
     * }
     */
    public static int Flags$get(MemorySegment seg) {
        return (int)constants$2679.const$0.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * DWORD Flags;
     * }
     */
    public static void Flags$set(MemorySegment seg, int x) {
        constants$2679.const$0.set(seg, x);
    }
    public static int Flags$get(MemorySegment seg, long index) {
        return (int)constants$2679.const$0.get(seg.asSlice(index*sizeof()));
    }
    public static void Flags$set(MemorySegment seg, long index, int x) {
        constants$2679.const$0.set(seg.asSlice(index*sizeof()), x);
    }
    public static MemorySegment Reserved$slice(MemorySegment seg) {
        return seg.asSlice(8, 8);
    }
    public static MemorySegment Cluster$slice(MemorySegment seg) {
        return seg.asSlice(16, 8);
    }
    public static MemorySegment FileName$slice(MemorySegment seg) {
        return seg.asSlice(24, 2);
    }
    public static long sizeof() { return $LAYOUT().byteSize(); }
    public static MemorySegment allocate(SegmentAllocator allocator) { return allocator.allocate($LAYOUT()); }
    public static MemorySegment allocateArray(long len, SegmentAllocator allocator) {
        return allocator.allocate(MemoryLayout.sequenceLayout(len, $LAYOUT()));
    }
    public static MemorySegment ofAddress(MemorySegment addr, Arena arena) { return RuntimeHelper.asArray(addr, $LAYOUT(), 1, arena); }
}


