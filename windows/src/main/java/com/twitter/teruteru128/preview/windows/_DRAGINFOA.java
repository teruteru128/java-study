// Generated by jextract

package com.twitter.teruteru128.preview.windows;

import java.lang.invoke.MethodHandle;
import java.lang.invoke.VarHandle;
import java.nio.ByteOrder;
import java.lang.foreign.*;
import static java.lang.foreign.ValueLayout.*;
/**
 * {@snippet :
 * struct _DRAGINFOA {
 *     UINT uSize;
 *     POINT pt;
 *     BOOL fNC;
 *     PZZSTR lpFileList;
 *     DWORD grfKeyState;
 * };
 * }
 */
public class _DRAGINFOA {

    public static MemoryLayout $LAYOUT() {
        return constants$1853.const$4;
    }
    public static VarHandle uSize$VH() {
        return constants$1853.const$5;
    }
    /**
     * Getter for field:
     * {@snippet :
     * UINT uSize;
     * }
     */
    public static int uSize$get(MemorySegment seg) {
        return (int)constants$1853.const$5.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * UINT uSize;
     * }
     */
    public static void uSize$set(MemorySegment seg, int x) {
        constants$1853.const$5.set(seg, x);
    }
    public static int uSize$get(MemorySegment seg, long index) {
        return (int)constants$1853.const$5.get(seg.asSlice(index*sizeof()));
    }
    public static void uSize$set(MemorySegment seg, long index, int x) {
        constants$1853.const$5.set(seg.asSlice(index*sizeof()), x);
    }
    public static MemorySegment pt$slice(MemorySegment seg) {
        return seg.asSlice(4, 8);
    }
    public static VarHandle fNC$VH() {
        return constants$1854.const$0;
    }
    /**
     * Getter for field:
     * {@snippet :
     * BOOL fNC;
     * }
     */
    public static int fNC$get(MemorySegment seg) {
        return (int)constants$1854.const$0.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * BOOL fNC;
     * }
     */
    public static void fNC$set(MemorySegment seg, int x) {
        constants$1854.const$0.set(seg, x);
    }
    public static int fNC$get(MemorySegment seg, long index) {
        return (int)constants$1854.const$0.get(seg.asSlice(index*sizeof()));
    }
    public static void fNC$set(MemorySegment seg, long index, int x) {
        constants$1854.const$0.set(seg.asSlice(index*sizeof()), x);
    }
    public static VarHandle lpFileList$VH() {
        return constants$1854.const$1;
    }
    /**
     * Getter for field:
     * {@snippet :
     * PZZSTR lpFileList;
     * }
     */
    public static MemorySegment lpFileList$get(MemorySegment seg) {
        return (java.lang.foreign.MemorySegment)constants$1854.const$1.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * PZZSTR lpFileList;
     * }
     */
    public static void lpFileList$set(MemorySegment seg, MemorySegment x) {
        constants$1854.const$1.set(seg, x);
    }
    public static MemorySegment lpFileList$get(MemorySegment seg, long index) {
        return (java.lang.foreign.MemorySegment)constants$1854.const$1.get(seg.asSlice(index*sizeof()));
    }
    public static void lpFileList$set(MemorySegment seg, long index, MemorySegment x) {
        constants$1854.const$1.set(seg.asSlice(index*sizeof()), x);
    }
    public static VarHandle grfKeyState$VH() {
        return constants$1854.const$2;
    }
    /**
     * Getter for field:
     * {@snippet :
     * DWORD grfKeyState;
     * }
     */
    public static int grfKeyState$get(MemorySegment seg) {
        return (int)constants$1854.const$2.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * DWORD grfKeyState;
     * }
     */
    public static void grfKeyState$set(MemorySegment seg, int x) {
        constants$1854.const$2.set(seg, x);
    }
    public static int grfKeyState$get(MemorySegment seg, long index) {
        return (int)constants$1854.const$2.get(seg.asSlice(index*sizeof()));
    }
    public static void grfKeyState$set(MemorySegment seg, long index, int x) {
        constants$1854.const$2.set(seg.asSlice(index*sizeof()), x);
    }
    public static long sizeof() { return $LAYOUT().byteSize(); }
    public static MemorySegment allocate(SegmentAllocator allocator) { return allocator.allocate($LAYOUT()); }
    public static MemorySegment allocateArray(long len, SegmentAllocator allocator) {
        return allocator.allocate(MemoryLayout.sequenceLayout(len, $LAYOUT()));
    }
    public static MemorySegment ofAddress(MemorySegment addr, Arena arena) { return RuntimeHelper.asArray(addr, $LAYOUT(), 1, arena); }
}


