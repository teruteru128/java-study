// Generated by jextract

package com.twitter.teruteru128.preview.windows;

import java.lang.invoke.MethodHandle;
import java.lang.invoke.VarHandle;
import java.nio.ByteOrder;
import java.lang.foreign.*;
import static java.lang.foreign.ValueLayout.*;
/**
 * {@snippet :
 * struct _POINTL {
 *     LONG x;
 *     LONG y;
 * };
 * }
 */
public class _POINTL {

    public static MemoryLayout $LAYOUT() {
        return constants$548.const$4;
    }
    public static VarHandle x$VH() {
        return constants$548.const$5;
    }
    /**
     * Getter for field:
     * {@snippet :
     * LONG x;
     * }
     */
    public static int x$get(MemorySegment seg) {
        return (int)constants$548.const$5.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * LONG x;
     * }
     */
    public static void x$set(MemorySegment seg, int x) {
        constants$548.const$5.set(seg, x);
    }
    public static int x$get(MemorySegment seg, long index) {
        return (int)constants$548.const$5.get(seg.asSlice(index*sizeof()));
    }
    public static void x$set(MemorySegment seg, long index, int x) {
        constants$548.const$5.set(seg.asSlice(index*sizeof()), x);
    }
    public static VarHandle y$VH() {
        return constants$549.const$0;
    }
    /**
     * Getter for field:
     * {@snippet :
     * LONG y;
     * }
     */
    public static int y$get(MemorySegment seg) {
        return (int)constants$549.const$0.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * LONG y;
     * }
     */
    public static void y$set(MemorySegment seg, int x) {
        constants$549.const$0.set(seg, x);
    }
    public static int y$get(MemorySegment seg, long index) {
        return (int)constants$549.const$0.get(seg.asSlice(index*sizeof()));
    }
    public static void y$set(MemorySegment seg, long index, int x) {
        constants$549.const$0.set(seg.asSlice(index*sizeof()), x);
    }
    public static long sizeof() { return $LAYOUT().byteSize(); }
    public static MemorySegment allocate(SegmentAllocator allocator) { return allocator.allocate($LAYOUT()); }
    public static MemorySegment allocateArray(long len, SegmentAllocator allocator) {
        return allocator.allocate(MemoryLayout.sequenceLayout(len, $LAYOUT()));
    }
    public static MemorySegment ofAddress(MemorySegment addr, Arena arena) { return RuntimeHelper.asArray(addr, $LAYOUT(), 1, arena); }
}


