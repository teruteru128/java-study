// Generated by jextract

package com.twitter.teruteru128.preview.windows;

import java.lang.invoke.MethodHandle;
import java.lang.invoke.VarHandle;
import java.nio.ByteOrder;
import java.lang.foreign.*;
import static java.lang.foreign.ValueLayout.*;
/**
 * {@snippet :
 * struct tagEMR {
 *     DWORD iType;
 *     DWORD nSize;
 * };
 * }
 */
public class tagEMR {

    public static MemoryLayout $LAYOUT() {
        return constants$1072.const$1;
    }
    public static VarHandle iType$VH() {
        return constants$1072.const$2;
    }
    /**
     * Getter for field:
     * {@snippet :
     * DWORD iType;
     * }
     */
    public static int iType$get(MemorySegment seg) {
        return (int)constants$1072.const$2.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * DWORD iType;
     * }
     */
    public static void iType$set(MemorySegment seg, int x) {
        constants$1072.const$2.set(seg, x);
    }
    public static int iType$get(MemorySegment seg, long index) {
        return (int)constants$1072.const$2.get(seg.asSlice(index*sizeof()));
    }
    public static void iType$set(MemorySegment seg, long index, int x) {
        constants$1072.const$2.set(seg.asSlice(index*sizeof()), x);
    }
    public static VarHandle nSize$VH() {
        return constants$1072.const$3;
    }
    /**
     * Getter for field:
     * {@snippet :
     * DWORD nSize;
     * }
     */
    public static int nSize$get(MemorySegment seg) {
        return (int)constants$1072.const$3.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * DWORD nSize;
     * }
     */
    public static void nSize$set(MemorySegment seg, int x) {
        constants$1072.const$3.set(seg, x);
    }
    public static int nSize$get(MemorySegment seg, long index) {
        return (int)constants$1072.const$3.get(seg.asSlice(index*sizeof()));
    }
    public static void nSize$set(MemorySegment seg, long index, int x) {
        constants$1072.const$3.set(seg.asSlice(index*sizeof()), x);
    }
    public static long sizeof() { return $LAYOUT().byteSize(); }
    public static MemorySegment allocate(SegmentAllocator allocator) { return allocator.allocate($LAYOUT()); }
    public static MemorySegment allocateArray(long len, SegmentAllocator allocator) {
        return allocator.allocate(MemoryLayout.sequenceLayout(len, $LAYOUT()));
    }
    public static MemorySegment ofAddress(MemorySegment addr, Arena arena) { return RuntimeHelper.asArray(addr, $LAYOUT(), 1, arena); }
}


