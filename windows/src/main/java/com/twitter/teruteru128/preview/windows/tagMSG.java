// Generated by jextract

package com.twitter.teruteru128.preview.windows;

import java.lang.invoke.MethodHandle;
import java.lang.invoke.VarHandle;
import java.nio.ByteOrder;
import java.lang.foreign.*;
import static java.lang.foreign.ValueLayout.*;
/**
 * {@snippet :
 * struct tagMSG {
 *     HWND hwnd;
 *     UINT message;
 *     WPARAM wParam;
 *     LPARAM lParam;
 *     DWORD time;
 *     POINT pt;
 * };
 * }
 */
public class tagMSG {

    public static MemoryLayout $LAYOUT() {
        return constants$1165.const$1;
    }
    public static VarHandle hwnd$VH() {
        return constants$1165.const$2;
    }
    /**
     * Getter for field:
     * {@snippet :
     * HWND hwnd;
     * }
     */
    public static MemorySegment hwnd$get(MemorySegment seg) {
        return (java.lang.foreign.MemorySegment)constants$1165.const$2.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * HWND hwnd;
     * }
     */
    public static void hwnd$set(MemorySegment seg, MemorySegment x) {
        constants$1165.const$2.set(seg, x);
    }
    public static MemorySegment hwnd$get(MemorySegment seg, long index) {
        return (java.lang.foreign.MemorySegment)constants$1165.const$2.get(seg.asSlice(index*sizeof()));
    }
    public static void hwnd$set(MemorySegment seg, long index, MemorySegment x) {
        constants$1165.const$2.set(seg.asSlice(index*sizeof()), x);
    }
    public static VarHandle message$VH() {
        return constants$1165.const$3;
    }
    /**
     * Getter for field:
     * {@snippet :
     * UINT message;
     * }
     */
    public static int message$get(MemorySegment seg) {
        return (int)constants$1165.const$3.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * UINT message;
     * }
     */
    public static void message$set(MemorySegment seg, int x) {
        constants$1165.const$3.set(seg, x);
    }
    public static int message$get(MemorySegment seg, long index) {
        return (int)constants$1165.const$3.get(seg.asSlice(index*sizeof()));
    }
    public static void message$set(MemorySegment seg, long index, int x) {
        constants$1165.const$3.set(seg.asSlice(index*sizeof()), x);
    }
    public static VarHandle wParam$VH() {
        return constants$1165.const$4;
    }
    /**
     * Getter for field:
     * {@snippet :
     * WPARAM wParam;
     * }
     */
    public static long wParam$get(MemorySegment seg) {
        return (long)constants$1165.const$4.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * WPARAM wParam;
     * }
     */
    public static void wParam$set(MemorySegment seg, long x) {
        constants$1165.const$4.set(seg, x);
    }
    public static long wParam$get(MemorySegment seg, long index) {
        return (long)constants$1165.const$4.get(seg.asSlice(index*sizeof()));
    }
    public static void wParam$set(MemorySegment seg, long index, long x) {
        constants$1165.const$4.set(seg.asSlice(index*sizeof()), x);
    }
    public static VarHandle lParam$VH() {
        return constants$1165.const$5;
    }
    /**
     * Getter for field:
     * {@snippet :
     * LPARAM lParam;
     * }
     */
    public static long lParam$get(MemorySegment seg) {
        return (long)constants$1165.const$5.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * LPARAM lParam;
     * }
     */
    public static void lParam$set(MemorySegment seg, long x) {
        constants$1165.const$5.set(seg, x);
    }
    public static long lParam$get(MemorySegment seg, long index) {
        return (long)constants$1165.const$5.get(seg.asSlice(index*sizeof()));
    }
    public static void lParam$set(MemorySegment seg, long index, long x) {
        constants$1165.const$5.set(seg.asSlice(index*sizeof()), x);
    }
    public static VarHandle time$VH() {
        return constants$1166.const$0;
    }
    /**
     * Getter for field:
     * {@snippet :
     * DWORD time;
     * }
     */
    public static int time$get(MemorySegment seg) {
        return (int)constants$1166.const$0.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * DWORD time;
     * }
     */
    public static void time$set(MemorySegment seg, int x) {
        constants$1166.const$0.set(seg, x);
    }
    public static int time$get(MemorySegment seg, long index) {
        return (int)constants$1166.const$0.get(seg.asSlice(index*sizeof()));
    }
    public static void time$set(MemorySegment seg, long index, int x) {
        constants$1166.const$0.set(seg.asSlice(index*sizeof()), x);
    }
    public static MemorySegment pt$slice(MemorySegment seg) {
        return seg.asSlice(36, 8);
    }
    public static long sizeof() { return $LAYOUT().byteSize(); }
    public static MemorySegment allocate(SegmentAllocator allocator) { return allocator.allocate($LAYOUT()); }
    public static MemorySegment allocateArray(long len, SegmentAllocator allocator) {
        return allocator.allocate(MemoryLayout.sequenceLayout(len, $LAYOUT()));
    }
    public static MemorySegment ofAddress(MemorySegment addr, Arena arena) { return RuntimeHelper.asArray(addr, $LAYOUT(), 1, arena); }
}


