// Generated by jextract

package com.twitter.teruteru128.preview.windows;

import java.lang.invoke.MethodHandle;
import java.lang.invoke.VarHandle;
import java.nio.ByteOrder;
import java.lang.foreign.*;
import static java.lang.foreign.ValueLayout.*;
/**
 * {@snippet :
 * struct _tagPROTOCOLDATA {
 *     DWORD grfFlags;
 *     DWORD dwState;
 *     LPVOID pData;
 *     ULONG cbData;
 * };
 * }
 */
public class _tagPROTOCOLDATA {

    public static MemoryLayout $LAYOUT() {
        return constants$4201.const$3;
    }
    public static VarHandle grfFlags$VH() {
        return constants$4201.const$4;
    }
    /**
     * Getter for field:
     * {@snippet :
     * DWORD grfFlags;
     * }
     */
    public static int grfFlags$get(MemorySegment seg) {
        return (int)constants$4201.const$4.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * DWORD grfFlags;
     * }
     */
    public static void grfFlags$set(MemorySegment seg, int x) {
        constants$4201.const$4.set(seg, x);
    }
    public static int grfFlags$get(MemorySegment seg, long index) {
        return (int)constants$4201.const$4.get(seg.asSlice(index*sizeof()));
    }
    public static void grfFlags$set(MemorySegment seg, long index, int x) {
        constants$4201.const$4.set(seg.asSlice(index*sizeof()), x);
    }
    public static VarHandle dwState$VH() {
        return constants$4201.const$5;
    }
    /**
     * Getter for field:
     * {@snippet :
     * DWORD dwState;
     * }
     */
    public static int dwState$get(MemorySegment seg) {
        return (int)constants$4201.const$5.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * DWORD dwState;
     * }
     */
    public static void dwState$set(MemorySegment seg, int x) {
        constants$4201.const$5.set(seg, x);
    }
    public static int dwState$get(MemorySegment seg, long index) {
        return (int)constants$4201.const$5.get(seg.asSlice(index*sizeof()));
    }
    public static void dwState$set(MemorySegment seg, long index, int x) {
        constants$4201.const$5.set(seg.asSlice(index*sizeof()), x);
    }
    public static VarHandle pData$VH() {
        return constants$4202.const$0;
    }
    /**
     * Getter for field:
     * {@snippet :
     * LPVOID pData;
     * }
     */
    public static MemorySegment pData$get(MemorySegment seg) {
        return (java.lang.foreign.MemorySegment)constants$4202.const$0.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * LPVOID pData;
     * }
     */
    public static void pData$set(MemorySegment seg, MemorySegment x) {
        constants$4202.const$0.set(seg, x);
    }
    public static MemorySegment pData$get(MemorySegment seg, long index) {
        return (java.lang.foreign.MemorySegment)constants$4202.const$0.get(seg.asSlice(index*sizeof()));
    }
    public static void pData$set(MemorySegment seg, long index, MemorySegment x) {
        constants$4202.const$0.set(seg.asSlice(index*sizeof()), x);
    }
    public static VarHandle cbData$VH() {
        return constants$4202.const$1;
    }
    /**
     * Getter for field:
     * {@snippet :
     * ULONG cbData;
     * }
     */
    public static int cbData$get(MemorySegment seg) {
        return (int)constants$4202.const$1.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * ULONG cbData;
     * }
     */
    public static void cbData$set(MemorySegment seg, int x) {
        constants$4202.const$1.set(seg, x);
    }
    public static int cbData$get(MemorySegment seg, long index) {
        return (int)constants$4202.const$1.get(seg.asSlice(index*sizeof()));
    }
    public static void cbData$set(MemorySegment seg, long index, int x) {
        constants$4202.const$1.set(seg.asSlice(index*sizeof()), x);
    }
    public static long sizeof() { return $LAYOUT().byteSize(); }
    public static MemorySegment allocate(SegmentAllocator allocator) { return allocator.allocate($LAYOUT()); }
    public static MemorySegment allocateArray(long len, SegmentAllocator allocator) {
        return allocator.allocate(MemoryLayout.sequenceLayout(len, $LAYOUT()));
    }
    public static MemorySegment ofAddress(MemorySegment addr, Arena arena) { return RuntimeHelper.asArray(addr, $LAYOUT(), 1, arena); }
}


