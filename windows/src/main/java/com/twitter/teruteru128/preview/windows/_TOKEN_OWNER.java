// Generated by jextract

package com.twitter.teruteru128.preview.windows;

import java.lang.invoke.MethodHandle;
import java.lang.invoke.VarHandle;
import java.nio.ByteOrder;
import java.lang.foreign.*;
import static java.lang.foreign.ValueLayout.*;
/**
 * {@snippet :
 * struct _TOKEN_OWNER {
 *     PSID Owner;
 * };
 * }
 */
public class _TOKEN_OWNER {

    public static MemoryLayout $LAYOUT() {
        return constants$165.const$4;
    }
    public static VarHandle Owner$VH() {
        return constants$165.const$5;
    }
    /**
     * Getter for field:
     * {@snippet :
     * PSID Owner;
     * }
     */
    public static MemorySegment Owner$get(MemorySegment seg) {
        return (java.lang.foreign.MemorySegment)constants$165.const$5.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * PSID Owner;
     * }
     */
    public static void Owner$set(MemorySegment seg, MemorySegment x) {
        constants$165.const$5.set(seg, x);
    }
    public static MemorySegment Owner$get(MemorySegment seg, long index) {
        return (java.lang.foreign.MemorySegment)constants$165.const$5.get(seg.asSlice(index*sizeof()));
    }
    public static void Owner$set(MemorySegment seg, long index, MemorySegment x) {
        constants$165.const$5.set(seg.asSlice(index*sizeof()), x);
    }
    public static long sizeof() { return $LAYOUT().byteSize(); }
    public static MemorySegment allocate(SegmentAllocator allocator) { return allocator.allocate($LAYOUT()); }
    public static MemorySegment allocateArray(long len, SegmentAllocator allocator) {
        return allocator.allocate(MemoryLayout.sequenceLayout(len, $LAYOUT()));
    }
    public static MemorySegment ofAddress(MemorySegment addr, Arena arena) { return RuntimeHelper.asArray(addr, $LAYOUT(), 1, arena); }
}


