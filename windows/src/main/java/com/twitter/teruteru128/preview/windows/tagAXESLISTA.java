// Generated by jextract

package com.twitter.teruteru128.preview.windows;

import java.lang.invoke.MethodHandle;
import java.lang.invoke.VarHandle;
import java.nio.ByteOrder;
import java.lang.foreign.*;
import static java.lang.foreign.ValueLayout.*;
/**
 * {@snippet :
 * struct tagAXESLISTA {
 *     DWORD axlReserved;
 *     DWORD axlNumAxes;
 *     AXISINFOA axlAxisInfo[16];
 * };
 * }
 */
public class tagAXESLISTA {

    public static MemoryLayout $LAYOUT() {
        return constants$1026.const$1;
    }
    public static VarHandle axlReserved$VH() {
        return constants$1026.const$2;
    }
    /**
     * Getter for field:
     * {@snippet :
     * DWORD axlReserved;
     * }
     */
    public static int axlReserved$get(MemorySegment seg) {
        return (int)constants$1026.const$2.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * DWORD axlReserved;
     * }
     */
    public static void axlReserved$set(MemorySegment seg, int x) {
        constants$1026.const$2.set(seg, x);
    }
    public static int axlReserved$get(MemorySegment seg, long index) {
        return (int)constants$1026.const$2.get(seg.asSlice(index*sizeof()));
    }
    public static void axlReserved$set(MemorySegment seg, long index, int x) {
        constants$1026.const$2.set(seg.asSlice(index*sizeof()), x);
    }
    public static VarHandle axlNumAxes$VH() {
        return constants$1026.const$3;
    }
    /**
     * Getter for field:
     * {@snippet :
     * DWORD axlNumAxes;
     * }
     */
    public static int axlNumAxes$get(MemorySegment seg) {
        return (int)constants$1026.const$3.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * DWORD axlNumAxes;
     * }
     */
    public static void axlNumAxes$set(MemorySegment seg, int x) {
        constants$1026.const$3.set(seg, x);
    }
    public static int axlNumAxes$get(MemorySegment seg, long index) {
        return (int)constants$1026.const$3.get(seg.asSlice(index*sizeof()));
    }
    public static void axlNumAxes$set(MemorySegment seg, long index, int x) {
        constants$1026.const$3.set(seg.asSlice(index*sizeof()), x);
    }
    public static MemorySegment axlAxisInfo$slice(MemorySegment seg) {
        return seg.asSlice(8, 384);
    }
    public static long sizeof() { return $LAYOUT().byteSize(); }
    public static MemorySegment allocate(SegmentAllocator allocator) { return allocator.allocate($LAYOUT()); }
    public static MemorySegment allocateArray(long len, SegmentAllocator allocator) {
        return allocator.allocate(MemoryLayout.sequenceLayout(len, $LAYOUT()));
    }
    public static MemorySegment ofAddress(MemorySegment addr, Arena arena) { return RuntimeHelper.asArray(addr, $LAYOUT(), 1, arena); }
}


