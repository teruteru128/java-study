// Generated by jextract

package com.github.teruteru.windows;

import java.lang.foreign.*;
import java.util.function.*;

import static java.lang.foreign.ValueLayout.*;
import static java.lang.foreign.MemoryLayout.PathElement.*;

/**
 * {@snippet lang=c :
 * struct tagRemFORMATETC {
 *     DWORD cfFormat;
 *     DWORD ptd;
 *     DWORD dwAspect;
 *     LONG lindex;
 *     DWORD tymed;
 * }
 * }
 */
public class tagRemFORMATETC {

    tagRemFORMATETC() {
        // Should not be called directly
    }

    private static final GroupLayout $LAYOUT = MemoryLayout.structLayout(
        Windows_h.C_LONG.withName("cfFormat"),
        Windows_h.C_LONG.withName("ptd"),
        Windows_h.C_LONG.withName("dwAspect"),
        Windows_h.C_LONG.withName("lindex"),
        Windows_h.C_LONG.withName("tymed")
    ).withName("tagRemFORMATETC");

    /**
     * The layout of this struct
     */
    public static final GroupLayout layout() {
        return $LAYOUT;
    }

    private static final OfInt cfFormat$LAYOUT = (OfInt)$LAYOUT.select(groupElement("cfFormat"));

    /**
     * Layout for field:
     * {@snippet lang=c :
     * DWORD cfFormat
     * }
     */
    public static final OfInt cfFormat$layout() {
        return cfFormat$LAYOUT;
    }

    private static final long cfFormat$OFFSET = 0;

    /**
     * Offset for field:
     * {@snippet lang=c :
     * DWORD cfFormat
     * }
     */
    public static final long cfFormat$offset() {
        return cfFormat$OFFSET;
    }

    /**
     * Getter for field:
     * {@snippet lang=c :
     * DWORD cfFormat
     * }
     */
    public static int cfFormat(MemorySegment struct) {
        return struct.get(cfFormat$LAYOUT, cfFormat$OFFSET);
    }

    /**
     * Setter for field:
     * {@snippet lang=c :
     * DWORD cfFormat
     * }
     */
    public static void cfFormat(MemorySegment struct, int fieldValue) {
        struct.set(cfFormat$LAYOUT, cfFormat$OFFSET, fieldValue);
    }

    private static final OfInt ptd$LAYOUT = (OfInt)$LAYOUT.select(groupElement("ptd"));

    /**
     * Layout for field:
     * {@snippet lang=c :
     * DWORD ptd
     * }
     */
    public static final OfInt ptd$layout() {
        return ptd$LAYOUT;
    }

    private static final long ptd$OFFSET = 4;

    /**
     * Offset for field:
     * {@snippet lang=c :
     * DWORD ptd
     * }
     */
    public static final long ptd$offset() {
        return ptd$OFFSET;
    }

    /**
     * Getter for field:
     * {@snippet lang=c :
     * DWORD ptd
     * }
     */
    public static int ptd(MemorySegment struct) {
        return struct.get(ptd$LAYOUT, ptd$OFFSET);
    }

    /**
     * Setter for field:
     * {@snippet lang=c :
     * DWORD ptd
     * }
     */
    public static void ptd(MemorySegment struct, int fieldValue) {
        struct.set(ptd$LAYOUT, ptd$OFFSET, fieldValue);
    }

    private static final OfInt dwAspect$LAYOUT = (OfInt)$LAYOUT.select(groupElement("dwAspect"));

    /**
     * Layout for field:
     * {@snippet lang=c :
     * DWORD dwAspect
     * }
     */
    public static final OfInt dwAspect$layout() {
        return dwAspect$LAYOUT;
    }

    private static final long dwAspect$OFFSET = 8;

    /**
     * Offset for field:
     * {@snippet lang=c :
     * DWORD dwAspect
     * }
     */
    public static final long dwAspect$offset() {
        return dwAspect$OFFSET;
    }

    /**
     * Getter for field:
     * {@snippet lang=c :
     * DWORD dwAspect
     * }
     */
    public static int dwAspect(MemorySegment struct) {
        return struct.get(dwAspect$LAYOUT, dwAspect$OFFSET);
    }

    /**
     * Setter for field:
     * {@snippet lang=c :
     * DWORD dwAspect
     * }
     */
    public static void dwAspect(MemorySegment struct, int fieldValue) {
        struct.set(dwAspect$LAYOUT, dwAspect$OFFSET, fieldValue);
    }

    private static final OfInt lindex$LAYOUT = (OfInt)$LAYOUT.select(groupElement("lindex"));

    /**
     * Layout for field:
     * {@snippet lang=c :
     * LONG lindex
     * }
     */
    public static final OfInt lindex$layout() {
        return lindex$LAYOUT;
    }

    private static final long lindex$OFFSET = 12;

    /**
     * Offset for field:
     * {@snippet lang=c :
     * LONG lindex
     * }
     */
    public static final long lindex$offset() {
        return lindex$OFFSET;
    }

    /**
     * Getter for field:
     * {@snippet lang=c :
     * LONG lindex
     * }
     */
    public static int lindex(MemorySegment struct) {
        return struct.get(lindex$LAYOUT, lindex$OFFSET);
    }

    /**
     * Setter for field:
     * {@snippet lang=c :
     * LONG lindex
     * }
     */
    public static void lindex(MemorySegment struct, int fieldValue) {
        struct.set(lindex$LAYOUT, lindex$OFFSET, fieldValue);
    }

    private static final OfInt tymed$LAYOUT = (OfInt)$LAYOUT.select(groupElement("tymed"));

    /**
     * Layout for field:
     * {@snippet lang=c :
     * DWORD tymed
     * }
     */
    public static final OfInt tymed$layout() {
        return tymed$LAYOUT;
    }

    private static final long tymed$OFFSET = 16;

    /**
     * Offset for field:
     * {@snippet lang=c :
     * DWORD tymed
     * }
     */
    public static final long tymed$offset() {
        return tymed$OFFSET;
    }

    /**
     * Getter for field:
     * {@snippet lang=c :
     * DWORD tymed
     * }
     */
    public static int tymed(MemorySegment struct) {
        return struct.get(tymed$LAYOUT, tymed$OFFSET);
    }

    /**
     * Setter for field:
     * {@snippet lang=c :
     * DWORD tymed
     * }
     */
    public static void tymed(MemorySegment struct, int fieldValue) {
        struct.set(tymed$LAYOUT, tymed$OFFSET, fieldValue);
    }

    /**
     * Obtains a slice of {@code arrayParam} which selects the array element at {@code index}.
     * The returned segment has address {@code arrayParam.address() + index * layout().byteSize()}
     */
    public static MemorySegment asSlice(MemorySegment array, long index) {
        return array.asSlice(layout().byteSize() * index);
    }

    /**
     * The size (in bytes) of this struct
     */
    public static long sizeof() { return layout().byteSize(); }

    /**
     * Allocate a segment of size {@code layout().byteSize()} using {@code allocator}
     */
    public static MemorySegment allocate(SegmentAllocator allocator) {
        return allocator.allocate(layout());
    }

    /**
     * Allocate an array of size {@code elementCount} using {@code allocator}.
     * The returned segment has size {@code elementCount * layout().byteSize()}.
     */
    public static MemorySegment allocateArray(long elementCount, SegmentAllocator allocator) {
        return allocator.allocate(MemoryLayout.sequenceLayout(elementCount, layout()));
    }

    /**
     * Reinterprets {@code addr} using target {@code arena} and {@code cleanupAction) (if any).
     * The returned segment has size {@code layout().byteSize()}
     */
    public static MemorySegment reinterpret(MemorySegment addr, Arena arena, Consumer<MemorySegment> cleanup) {
        return reinterpret(addr, 1, arena, cleanup);
    }

    /**
     * Reinterprets {@code addr} using target {@code arena} and {@code cleanupAction) (if any).
     * The returned segment has size {@code elementCount * layout().byteSize()}
     */
    public static MemorySegment reinterpret(MemorySegment addr, long elementCount, Arena arena, Consumer<MemorySegment> cleanup) {
        return addr.reinterpret(layout().byteSize() * elementCount, arena, cleanup);
    }
}

