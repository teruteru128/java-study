// Generated by jextract

package com.twitter.teruteru128.preview.windows;

import java.lang.invoke.MethodHandle;
import java.lang.invoke.VarHandle;
import java.nio.ByteOrder;
import java.lang.foreign.*;
import static java.lang.foreign.ValueLayout.*;
/**
 * {@snippet :
 * struct _WGLSWAP {
 *     HDC hdc;
 *     UINT uiFlags;
 * };
 * }
 */
public class _WGLSWAP {

    public static MemoryLayout $LAYOUT() {
        return constants$1131.const$0;
    }
    public static VarHandle hdc$VH() {
        return constants$1131.const$1;
    }
    /**
     * Getter for field:
     * {@snippet :
     * HDC hdc;
     * }
     */
    public static MemorySegment hdc$get(MemorySegment seg) {
        return (java.lang.foreign.MemorySegment)constants$1131.const$1.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * HDC hdc;
     * }
     */
    public static void hdc$set(MemorySegment seg, MemorySegment x) {
        constants$1131.const$1.set(seg, x);
    }
    public static MemorySegment hdc$get(MemorySegment seg, long index) {
        return (java.lang.foreign.MemorySegment)constants$1131.const$1.get(seg.asSlice(index*sizeof()));
    }
    public static void hdc$set(MemorySegment seg, long index, MemorySegment x) {
        constants$1131.const$1.set(seg.asSlice(index*sizeof()), x);
    }
    public static VarHandle uiFlags$VH() {
        return constants$1131.const$2;
    }
    /**
     * Getter for field:
     * {@snippet :
     * UINT uiFlags;
     * }
     */
    public static int uiFlags$get(MemorySegment seg) {
        return (int)constants$1131.const$2.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * UINT uiFlags;
     * }
     */
    public static void uiFlags$set(MemorySegment seg, int x) {
        constants$1131.const$2.set(seg, x);
    }
    public static int uiFlags$get(MemorySegment seg, long index) {
        return (int)constants$1131.const$2.get(seg.asSlice(index*sizeof()));
    }
    public static void uiFlags$set(MemorySegment seg, long index, int x) {
        constants$1131.const$2.set(seg.asSlice(index*sizeof()), x);
    }
    public static long sizeof() { return $LAYOUT().byteSize(); }
    public static MemorySegment allocate(SegmentAllocator allocator) { return allocator.allocate($LAYOUT()); }
    public static MemorySegment allocateArray(long len, SegmentAllocator allocator) {
        return allocator.allocate(MemoryLayout.sequenceLayout(len, $LAYOUT()));
    }
    public static MemorySegment ofAddress(MemorySegment addr, Arena arena) { return RuntimeHelper.asArray(addr, $LAYOUT(), 1, arena); }
}


