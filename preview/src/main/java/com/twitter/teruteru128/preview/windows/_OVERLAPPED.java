// Generated by jextract

package com.twitter.teruteru128.preview.windows;

import java.lang.invoke.MethodHandle;
import java.lang.invoke.VarHandle;
import java.nio.ByteOrder;
import java.lang.foreign.*;
import static java.lang.foreign.ValueLayout.*;
/**
 * {@snippet :
 * struct _OVERLAPPED {
 *     ULONG_PTR Internal;
 *     ULONG_PTR InternalHigh;
 *     union {
 *         struct {
 *             DWORD Offset;
 *             DWORD OffsetHigh;
 *         };
 *         PVOID Pointer;
 *     };
 *     HANDLE hEvent;
 * };
 * }
 */
public class _OVERLAPPED {

    public static MemoryLayout $LAYOUT() {
        return constants$551.const$2;
    }
    public static VarHandle Internal$VH() {
        return constants$551.const$3;
    }
    /**
     * Getter for field:
     * {@snippet :
     * ULONG_PTR Internal;
     * }
     */
    public static long Internal$get(MemorySegment seg) {
        return (long)constants$551.const$3.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * ULONG_PTR Internal;
     * }
     */
    public static void Internal$set(MemorySegment seg, long x) {
        constants$551.const$3.set(seg, x);
    }
    public static long Internal$get(MemorySegment seg, long index) {
        return (long)constants$551.const$3.get(seg.asSlice(index*sizeof()));
    }
    public static void Internal$set(MemorySegment seg, long index, long x) {
        constants$551.const$3.set(seg.asSlice(index*sizeof()), x);
    }
    public static VarHandle InternalHigh$VH() {
        return constants$551.const$4;
    }
    /**
     * Getter for field:
     * {@snippet :
     * ULONG_PTR InternalHigh;
     * }
     */
    public static long InternalHigh$get(MemorySegment seg) {
        return (long)constants$551.const$4.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * ULONG_PTR InternalHigh;
     * }
     */
    public static void InternalHigh$set(MemorySegment seg, long x) {
        constants$551.const$4.set(seg, x);
    }
    public static long InternalHigh$get(MemorySegment seg, long index) {
        return (long)constants$551.const$4.get(seg.asSlice(index*sizeof()));
    }
    public static void InternalHigh$set(MemorySegment seg, long index, long x) {
        constants$551.const$4.set(seg.asSlice(index*sizeof()), x);
    }
    public static VarHandle Offset$VH() {
        return constants$551.const$5;
    }
    /**
     * Getter for field:
     * {@snippet :
     * DWORD Offset;
     * }
     */
    public static int Offset$get(MemorySegment seg) {
        return (int)constants$551.const$5.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * DWORD Offset;
     * }
     */
    public static void Offset$set(MemorySegment seg, int x) {
        constants$551.const$5.set(seg, x);
    }
    public static int Offset$get(MemorySegment seg, long index) {
        return (int)constants$551.const$5.get(seg.asSlice(index*sizeof()));
    }
    public static void Offset$set(MemorySegment seg, long index, int x) {
        constants$551.const$5.set(seg.asSlice(index*sizeof()), x);
    }
    public static VarHandle OffsetHigh$VH() {
        return constants$552.const$0;
    }
    /**
     * Getter for field:
     * {@snippet :
     * DWORD OffsetHigh;
     * }
     */
    public static int OffsetHigh$get(MemorySegment seg) {
        return (int)constants$552.const$0.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * DWORD OffsetHigh;
     * }
     */
    public static void OffsetHigh$set(MemorySegment seg, int x) {
        constants$552.const$0.set(seg, x);
    }
    public static int OffsetHigh$get(MemorySegment seg, long index) {
        return (int)constants$552.const$0.get(seg.asSlice(index*sizeof()));
    }
    public static void OffsetHigh$set(MemorySegment seg, long index, int x) {
        constants$552.const$0.set(seg.asSlice(index*sizeof()), x);
    }
    public static VarHandle Pointer$VH() {
        return constants$552.const$1;
    }
    /**
     * Getter for field:
     * {@snippet :
     * PVOID Pointer;
     * }
     */
    public static MemorySegment Pointer$get(MemorySegment seg) {
        return (java.lang.foreign.MemorySegment)constants$552.const$1.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * PVOID Pointer;
     * }
     */
    public static void Pointer$set(MemorySegment seg, MemorySegment x) {
        constants$552.const$1.set(seg, x);
    }
    public static MemorySegment Pointer$get(MemorySegment seg, long index) {
        return (java.lang.foreign.MemorySegment)constants$552.const$1.get(seg.asSlice(index*sizeof()));
    }
    public static void Pointer$set(MemorySegment seg, long index, MemorySegment x) {
        constants$552.const$1.set(seg.asSlice(index*sizeof()), x);
    }
    public static VarHandle hEvent$VH() {
        return constants$552.const$2;
    }
    /**
     * Getter for field:
     * {@snippet :
     * HANDLE hEvent;
     * }
     */
    public static MemorySegment hEvent$get(MemorySegment seg) {
        return (java.lang.foreign.MemorySegment)constants$552.const$2.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * HANDLE hEvent;
     * }
     */
    public static void hEvent$set(MemorySegment seg, MemorySegment x) {
        constants$552.const$2.set(seg, x);
    }
    public static MemorySegment hEvent$get(MemorySegment seg, long index) {
        return (java.lang.foreign.MemorySegment)constants$552.const$2.get(seg.asSlice(index*sizeof()));
    }
    public static void hEvent$set(MemorySegment seg, long index, MemorySegment x) {
        constants$552.const$2.set(seg.asSlice(index*sizeof()), x);
    }
    public static long sizeof() { return $LAYOUT().byteSize(); }
    public static MemorySegment allocate(SegmentAllocator allocator) { return allocator.allocate($LAYOUT()); }
    public static MemorySegment allocateArray(long len, SegmentAllocator allocator) {
        return allocator.allocate(MemoryLayout.sequenceLayout(len, $LAYOUT()));
    }
    public static MemorySegment ofAddress(MemorySegment addr, Arena arena) { return RuntimeHelper.asArray(addr, $LAYOUT(), 1, arena); }
}


