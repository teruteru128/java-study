// Generated by jextract

package com.twitter.teruteru128.preview.windows;

import java.lang.invoke.MethodHandle;
import java.lang.invoke.VarHandle;
import java.nio.ByteOrder;
import java.lang.foreign.*;
import static java.lang.foreign.ValueLayout.*;
/**
 * {@snippet :
 * struct tagPROPSPEC {
 *     ULONG ulKind;
 *     union {
 *         PROPID propid;
 *         LPOLESTR lpwstr;
 *     };
 * };
 * }
 */
public class tagPROPSPEC {

    public static MemoryLayout $LAYOUT() {
        return constants$3603.const$2;
    }
    public static VarHandle ulKind$VH() {
        return constants$3603.const$3;
    }
    /**
     * Getter for field:
     * {@snippet :
     * ULONG ulKind;
     * }
     */
    public static int ulKind$get(MemorySegment seg) {
        return (int)constants$3603.const$3.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * ULONG ulKind;
     * }
     */
    public static void ulKind$set(MemorySegment seg, int x) {
        constants$3603.const$3.set(seg, x);
    }
    public static int ulKind$get(MemorySegment seg, long index) {
        return (int)constants$3603.const$3.get(seg.asSlice(index*sizeof()));
    }
    public static void ulKind$set(MemorySegment seg, long index, int x) {
        constants$3603.const$3.set(seg.asSlice(index*sizeof()), x);
    }
    public static VarHandle propid$VH() {
        return constants$3603.const$4;
    }
    /**
     * Getter for field:
     * {@snippet :
     * PROPID propid;
     * }
     */
    public static int propid$get(MemorySegment seg) {
        return (int)constants$3603.const$4.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * PROPID propid;
     * }
     */
    public static void propid$set(MemorySegment seg, int x) {
        constants$3603.const$4.set(seg, x);
    }
    public static int propid$get(MemorySegment seg, long index) {
        return (int)constants$3603.const$4.get(seg.asSlice(index*sizeof()));
    }
    public static void propid$set(MemorySegment seg, long index, int x) {
        constants$3603.const$4.set(seg.asSlice(index*sizeof()), x);
    }
    public static VarHandle lpwstr$VH() {
        return constants$3603.const$5;
    }
    /**
     * Getter for field:
     * {@snippet :
     * LPOLESTR lpwstr;
     * }
     */
    public static MemorySegment lpwstr$get(MemorySegment seg) {
        return (java.lang.foreign.MemorySegment)constants$3603.const$5.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * LPOLESTR lpwstr;
     * }
     */
    public static void lpwstr$set(MemorySegment seg, MemorySegment x) {
        constants$3603.const$5.set(seg, x);
    }
    public static MemorySegment lpwstr$get(MemorySegment seg, long index) {
        return (java.lang.foreign.MemorySegment)constants$3603.const$5.get(seg.asSlice(index*sizeof()));
    }
    public static void lpwstr$set(MemorySegment seg, long index, MemorySegment x) {
        constants$3603.const$5.set(seg.asSlice(index*sizeof()), x);
    }
    public static long sizeof() { return $LAYOUT().byteSize(); }
    public static MemorySegment allocate(SegmentAllocator allocator) { return allocator.allocate($LAYOUT()); }
    public static MemorySegment allocateArray(long len, SegmentAllocator allocator) {
        return allocator.allocate(MemoryLayout.sequenceLayout(len, $LAYOUT()));
    }
    public static MemorySegment ofAddress(MemorySegment addr, Arena arena) { return RuntimeHelper.asArray(addr, $LAYOUT(), 1, arena); }
}


