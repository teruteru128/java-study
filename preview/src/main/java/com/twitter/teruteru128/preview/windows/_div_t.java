// Generated by jextract

package com.twitter.teruteru128.preview.windows;

import java.lang.invoke.MethodHandle;
import java.lang.invoke.VarHandle;
import java.nio.ByteOrder;
import java.lang.foreign.*;
import static java.lang.foreign.ValueLayout.*;
/**
 * {@snippet :
 * struct _div_t {
 *     int quot;
 *     int rem;
 * };
 * }
 */
public class _div_t {

    public static MemoryLayout $LAYOUT() {
        return constants$2973.const$5;
    }
    public static VarHandle quot$VH() {
        return constants$2974.const$0;
    }
    /**
     * Getter for field:
     * {@snippet :
     * int quot;
     * }
     */
    public static int quot$get(MemorySegment seg) {
        return (int)constants$2974.const$0.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * int quot;
     * }
     */
    public static void quot$set(MemorySegment seg, int x) {
        constants$2974.const$0.set(seg, x);
    }
    public static int quot$get(MemorySegment seg, long index) {
        return (int)constants$2974.const$0.get(seg.asSlice(index*sizeof()));
    }
    public static void quot$set(MemorySegment seg, long index, int x) {
        constants$2974.const$0.set(seg.asSlice(index*sizeof()), x);
    }
    public static VarHandle rem$VH() {
        return constants$2974.const$1;
    }
    /**
     * Getter for field:
     * {@snippet :
     * int rem;
     * }
     */
    public static int rem$get(MemorySegment seg) {
        return (int)constants$2974.const$1.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * int rem;
     * }
     */
    public static void rem$set(MemorySegment seg, int x) {
        constants$2974.const$1.set(seg, x);
    }
    public static int rem$get(MemorySegment seg, long index) {
        return (int)constants$2974.const$1.get(seg.asSlice(index*sizeof()));
    }
    public static void rem$set(MemorySegment seg, long index, int x) {
        constants$2974.const$1.set(seg.asSlice(index*sizeof()), x);
    }
    public static long sizeof() { return $LAYOUT().byteSize(); }
    public static MemorySegment allocate(SegmentAllocator allocator) { return allocator.allocate($LAYOUT()); }
    public static MemorySegment allocateArray(long len, SegmentAllocator allocator) {
        return allocator.allocate(MemoryLayout.sequenceLayout(len, $LAYOUT()));
    }
    public static MemorySegment ofAddress(MemorySegment addr, Arena arena) { return RuntimeHelper.asArray(addr, $LAYOUT(), 1, arena); }
}


