// Generated by jextract

package com.twitter.teruteru128.preview.opencl;

import java.lang.invoke.MethodHandle;
import java.lang.invoke.VarHandle;
import java.nio.ByteOrder;
import java.lang.foreign.*;
import static java.lang.foreign.ValueLayout.*;
/**
 * {@snippet :
 * union {
 *     cl_double s[2];
 *     struct {
 *         cl_double x;
 *         cl_double y;
 *     };
 *     struct {
 *         cl_double s0;
 *         cl_double s1;
 *     };
 *     struct {
 *         cl_double lo;
 *         cl_double hi;
 *     };
 *     __cl_double2 v2;
 * };
 * }
 */
public class cl_double2 {

    public static MemoryLayout $LAYOUT() {
        return constants$162.const$4;
    }
    public static MemorySegment s$slice(MemorySegment seg) {
        return seg.asSlice(0, 16);
    }
    public static VarHandle x$VH() {
        return constants$162.const$5;
    }
    /**
     * Getter for field:
     * {@snippet :
     * cl_double x;
     * }
     */
    public static double x$get(MemorySegment seg) {
        return (double)constants$162.const$5.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * cl_double x;
     * }
     */
    public static void x$set(MemorySegment seg, double x) {
        constants$162.const$5.set(seg, x);
    }
    public static double x$get(MemorySegment seg, long index) {
        return (double)constants$162.const$5.get(seg.asSlice(index*sizeof()));
    }
    public static void x$set(MemorySegment seg, long index, double x) {
        constants$162.const$5.set(seg.asSlice(index*sizeof()), x);
    }
    public static VarHandle y$VH() {
        return constants$163.const$0;
    }
    /**
     * Getter for field:
     * {@snippet :
     * cl_double y;
     * }
     */
    public static double y$get(MemorySegment seg) {
        return (double)constants$163.const$0.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * cl_double y;
     * }
     */
    public static void y$set(MemorySegment seg, double x) {
        constants$163.const$0.set(seg, x);
    }
    public static double y$get(MemorySegment seg, long index) {
        return (double)constants$163.const$0.get(seg.asSlice(index*sizeof()));
    }
    public static void y$set(MemorySegment seg, long index, double x) {
        constants$163.const$0.set(seg.asSlice(index*sizeof()), x);
    }
    public static VarHandle s0$VH() {
        return constants$163.const$1;
    }
    /**
     * Getter for field:
     * {@snippet :
     * cl_double s0;
     * }
     */
    public static double s0$get(MemorySegment seg) {
        return (double)constants$163.const$1.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * cl_double s0;
     * }
     */
    public static void s0$set(MemorySegment seg, double x) {
        constants$163.const$1.set(seg, x);
    }
    public static double s0$get(MemorySegment seg, long index) {
        return (double)constants$163.const$1.get(seg.asSlice(index*sizeof()));
    }
    public static void s0$set(MemorySegment seg, long index, double x) {
        constants$163.const$1.set(seg.asSlice(index*sizeof()), x);
    }
    public static VarHandle s1$VH() {
        return constants$163.const$2;
    }
    /**
     * Getter for field:
     * {@snippet :
     * cl_double s1;
     * }
     */
    public static double s1$get(MemorySegment seg) {
        return (double)constants$163.const$2.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * cl_double s1;
     * }
     */
    public static void s1$set(MemorySegment seg, double x) {
        constants$163.const$2.set(seg, x);
    }
    public static double s1$get(MemorySegment seg, long index) {
        return (double)constants$163.const$2.get(seg.asSlice(index*sizeof()));
    }
    public static void s1$set(MemorySegment seg, long index, double x) {
        constants$163.const$2.set(seg.asSlice(index*sizeof()), x);
    }
    public static VarHandle lo$VH() {
        return constants$163.const$3;
    }
    /**
     * Getter for field:
     * {@snippet :
     * cl_double lo;
     * }
     */
    public static double lo$get(MemorySegment seg) {
        return (double)constants$163.const$3.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * cl_double lo;
     * }
     */
    public static void lo$set(MemorySegment seg, double x) {
        constants$163.const$3.set(seg, x);
    }
    public static double lo$get(MemorySegment seg, long index) {
        return (double)constants$163.const$3.get(seg.asSlice(index*sizeof()));
    }
    public static void lo$set(MemorySegment seg, long index, double x) {
        constants$163.const$3.set(seg.asSlice(index*sizeof()), x);
    }
    public static VarHandle hi$VH() {
        return constants$163.const$4;
    }
    /**
     * Getter for field:
     * {@snippet :
     * cl_double hi;
     * }
     */
    public static double hi$get(MemorySegment seg) {
        return (double)constants$163.const$4.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * cl_double hi;
     * }
     */
    public static void hi$set(MemorySegment seg, double x) {
        constants$163.const$4.set(seg, x);
    }
    public static double hi$get(MemorySegment seg, long index) {
        return (double)constants$163.const$4.get(seg.asSlice(index*sizeof()));
    }
    public static void hi$set(MemorySegment seg, long index, double x) {
        constants$163.const$4.set(seg.asSlice(index*sizeof()), x);
    }
    public static MemorySegment v2$slice(MemorySegment seg) {
        return seg.asSlice(0, 16);
    }
    public static long sizeof() { return $LAYOUT().byteSize(); }
    public static MemorySegment allocate(SegmentAllocator allocator) { return allocator.allocate($LAYOUT()); }
    public static MemorySegment allocateArray(long len, SegmentAllocator allocator) {
        return allocator.allocate(MemoryLayout.sequenceLayout(len, $LAYOUT()));
    }
    public static MemorySegment ofAddress(MemorySegment addr, Arena arena) { return RuntimeHelper.asArray(addr, $LAYOUT(), 1, arena); }
}


