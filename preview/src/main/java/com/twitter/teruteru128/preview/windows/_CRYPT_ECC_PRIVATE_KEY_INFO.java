// Generated by jextract

package com.twitter.teruteru128.preview.windows;

import java.lang.invoke.MethodHandle;
import java.lang.invoke.VarHandle;
import java.nio.ByteOrder;
import java.lang.foreign.*;
import static java.lang.foreign.ValueLayout.*;
/**
 * {@snippet :
 * struct _CRYPT_ECC_PRIVATE_KEY_INFO {
 *     DWORD dwVersion;
 *     CRYPT_DER_BLOB PrivateKey;
 *     LPSTR szCurveOid;
 *     CRYPT_BIT_BLOB PublicKey;
 * };
 * }
 */
public class _CRYPT_ECC_PRIVATE_KEY_INFO {

    public static MemoryLayout $LAYOUT() {
        return constants$2008.const$4;
    }
    public static VarHandle dwVersion$VH() {
        return constants$2008.const$5;
    }
    /**
     * Getter for field:
     * {@snippet :
     * DWORD dwVersion;
     * }
     */
    public static int dwVersion$get(MemorySegment seg) {
        return (int)constants$2008.const$5.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * DWORD dwVersion;
     * }
     */
    public static void dwVersion$set(MemorySegment seg, int x) {
        constants$2008.const$5.set(seg, x);
    }
    public static int dwVersion$get(MemorySegment seg, long index) {
        return (int)constants$2008.const$5.get(seg.asSlice(index*sizeof()));
    }
    public static void dwVersion$set(MemorySegment seg, long index, int x) {
        constants$2008.const$5.set(seg.asSlice(index*sizeof()), x);
    }
    public static MemorySegment PrivateKey$slice(MemorySegment seg) {
        return seg.asSlice(8, 16);
    }
    public static VarHandle szCurveOid$VH() {
        return constants$2009.const$0;
    }
    /**
     * Getter for field:
     * {@snippet :
     * LPSTR szCurveOid;
     * }
     */
    public static MemorySegment szCurveOid$get(MemorySegment seg) {
        return (java.lang.foreign.MemorySegment)constants$2009.const$0.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * LPSTR szCurveOid;
     * }
     */
    public static void szCurveOid$set(MemorySegment seg, MemorySegment x) {
        constants$2009.const$0.set(seg, x);
    }
    public static MemorySegment szCurveOid$get(MemorySegment seg, long index) {
        return (java.lang.foreign.MemorySegment)constants$2009.const$0.get(seg.asSlice(index*sizeof()));
    }
    public static void szCurveOid$set(MemorySegment seg, long index, MemorySegment x) {
        constants$2009.const$0.set(seg.asSlice(index*sizeof()), x);
    }
    public static MemorySegment PublicKey$slice(MemorySegment seg) {
        return seg.asSlice(32, 24);
    }
    public static long sizeof() { return $LAYOUT().byteSize(); }
    public static MemorySegment allocate(SegmentAllocator allocator) { return allocator.allocate($LAYOUT()); }
    public static MemorySegment allocateArray(long len, SegmentAllocator allocator) {
        return allocator.allocate(MemoryLayout.sequenceLayout(len, $LAYOUT()));
    }
    public static MemorySegment ofAddress(MemorySegment addr, Arena arena) { return RuntimeHelper.asArray(addr, $LAYOUT(), 1, arena); }
}


