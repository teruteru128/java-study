// Generated by jextract

package com.twitter.teruteru128.preview.opencl;

import java.lang.invoke.MethodHandle;
import java.lang.invoke.VarHandle;
import java.nio.ByteOrder;
import java.lang.foreign.*;
import static java.lang.foreign.ValueLayout.*;
/**
 * {@snippet :
 * union {
 *     cl_short s[2];
 *     struct {
 *         cl_short x;
 *         cl_short y;
 *     };
 *     struct {
 *         cl_short s0;
 *         cl_short s1;
 *     };
 *     struct {
 *         cl_short lo;
 *         cl_short hi;
 *     };
 * };
 * }
 */
public class cl_short2 {

    public static MemoryLayout $LAYOUT() {
        return constants$82.const$1;
    }
    public static MemorySegment s$slice(MemorySegment seg) {
        return seg.asSlice(0, 4);
    }
    public static VarHandle x$VH() {
        return constants$82.const$2;
    }
    /**
     * Getter for field:
     * {@snippet :
     * cl_short x;
     * }
     */
    public static short x$get(MemorySegment seg) {
        return (short)constants$82.const$2.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * cl_short x;
     * }
     */
    public static void x$set(MemorySegment seg, short x) {
        constants$82.const$2.set(seg, x);
    }
    public static short x$get(MemorySegment seg, long index) {
        return (short)constants$82.const$2.get(seg.asSlice(index*sizeof()));
    }
    public static void x$set(MemorySegment seg, long index, short x) {
        constants$82.const$2.set(seg.asSlice(index*sizeof()), x);
    }
    public static VarHandle y$VH() {
        return constants$82.const$3;
    }
    /**
     * Getter for field:
     * {@snippet :
     * cl_short y;
     * }
     */
    public static short y$get(MemorySegment seg) {
        return (short)constants$82.const$3.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * cl_short y;
     * }
     */
    public static void y$set(MemorySegment seg, short x) {
        constants$82.const$3.set(seg, x);
    }
    public static short y$get(MemorySegment seg, long index) {
        return (short)constants$82.const$3.get(seg.asSlice(index*sizeof()));
    }
    public static void y$set(MemorySegment seg, long index, short x) {
        constants$82.const$3.set(seg.asSlice(index*sizeof()), x);
    }
    public static VarHandle s0$VH() {
        return constants$82.const$4;
    }
    /**
     * Getter for field:
     * {@snippet :
     * cl_short s0;
     * }
     */
    public static short s0$get(MemorySegment seg) {
        return (short)constants$82.const$4.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * cl_short s0;
     * }
     */
    public static void s0$set(MemorySegment seg, short x) {
        constants$82.const$4.set(seg, x);
    }
    public static short s0$get(MemorySegment seg, long index) {
        return (short)constants$82.const$4.get(seg.asSlice(index*sizeof()));
    }
    public static void s0$set(MemorySegment seg, long index, short x) {
        constants$82.const$4.set(seg.asSlice(index*sizeof()), x);
    }
    public static VarHandle s1$VH() {
        return constants$82.const$5;
    }
    /**
     * Getter for field:
     * {@snippet :
     * cl_short s1;
     * }
     */
    public static short s1$get(MemorySegment seg) {
        return (short)constants$82.const$5.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * cl_short s1;
     * }
     */
    public static void s1$set(MemorySegment seg, short x) {
        constants$82.const$5.set(seg, x);
    }
    public static short s1$get(MemorySegment seg, long index) {
        return (short)constants$82.const$5.get(seg.asSlice(index*sizeof()));
    }
    public static void s1$set(MemorySegment seg, long index, short x) {
        constants$82.const$5.set(seg.asSlice(index*sizeof()), x);
    }
    public static VarHandle lo$VH() {
        return constants$83.const$0;
    }
    /**
     * Getter for field:
     * {@snippet :
     * cl_short lo;
     * }
     */
    public static short lo$get(MemorySegment seg) {
        return (short)constants$83.const$0.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * cl_short lo;
     * }
     */
    public static void lo$set(MemorySegment seg, short x) {
        constants$83.const$0.set(seg, x);
    }
    public static short lo$get(MemorySegment seg, long index) {
        return (short)constants$83.const$0.get(seg.asSlice(index*sizeof()));
    }
    public static void lo$set(MemorySegment seg, long index, short x) {
        constants$83.const$0.set(seg.asSlice(index*sizeof()), x);
    }
    public static VarHandle hi$VH() {
        return constants$83.const$1;
    }
    /**
     * Getter for field:
     * {@snippet :
     * cl_short hi;
     * }
     */
    public static short hi$get(MemorySegment seg) {
        return (short)constants$83.const$1.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * cl_short hi;
     * }
     */
    public static void hi$set(MemorySegment seg, short x) {
        constants$83.const$1.set(seg, x);
    }
    public static short hi$get(MemorySegment seg, long index) {
        return (short)constants$83.const$1.get(seg.asSlice(index*sizeof()));
    }
    public static void hi$set(MemorySegment seg, long index, short x) {
        constants$83.const$1.set(seg.asSlice(index*sizeof()), x);
    }
    public static long sizeof() { return $LAYOUT().byteSize(); }
    public static MemorySegment allocate(SegmentAllocator allocator) { return allocator.allocate($LAYOUT()); }
    public static MemorySegment allocateArray(long len, SegmentAllocator allocator) {
        return allocator.allocate(MemoryLayout.sequenceLayout(len, $LAYOUT()));
    }
    public static MemorySegment ofAddress(MemorySegment addr, Arena arena) { return RuntimeHelper.asArray(addr, $LAYOUT(), 1, arena); }
}


