// Generated by jextract

package com.twitter.teruteru128.preview.windows;

import java.lang.invoke.MethodHandle;
import java.lang.invoke.VarHandle;
import java.nio.ByteOrder;
import java.lang.foreign.*;
import static java.lang.foreign.ValueLayout.*;
/**
 * {@snippet :
 * struct _BCRYPT_PSS_PADDING_INFO {
 *     LPCWSTR pszAlgId;
 *     ULONG cbSalt;
 * };
 * }
 */
public class _BCRYPT_PSS_PADDING_INFO {

    public static MemoryLayout $LAYOUT() {
        return constants$1940.const$4;
    }
    public static VarHandle pszAlgId$VH() {
        return constants$1940.const$5;
    }
    /**
     * Getter for field:
     * {@snippet :
     * LPCWSTR pszAlgId;
     * }
     */
    public static MemorySegment pszAlgId$get(MemorySegment seg) {
        return (java.lang.foreign.MemorySegment)constants$1940.const$5.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * LPCWSTR pszAlgId;
     * }
     */
    public static void pszAlgId$set(MemorySegment seg, MemorySegment x) {
        constants$1940.const$5.set(seg, x);
    }
    public static MemorySegment pszAlgId$get(MemorySegment seg, long index) {
        return (java.lang.foreign.MemorySegment)constants$1940.const$5.get(seg.asSlice(index*sizeof()));
    }
    public static void pszAlgId$set(MemorySegment seg, long index, MemorySegment x) {
        constants$1940.const$5.set(seg.asSlice(index*sizeof()), x);
    }
    public static VarHandle cbSalt$VH() {
        return constants$1941.const$0;
    }
    /**
     * Getter for field:
     * {@snippet :
     * ULONG cbSalt;
     * }
     */
    public static int cbSalt$get(MemorySegment seg) {
        return (int)constants$1941.const$0.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * ULONG cbSalt;
     * }
     */
    public static void cbSalt$set(MemorySegment seg, int x) {
        constants$1941.const$0.set(seg, x);
    }
    public static int cbSalt$get(MemorySegment seg, long index) {
        return (int)constants$1941.const$0.get(seg.asSlice(index*sizeof()));
    }
    public static void cbSalt$set(MemorySegment seg, long index, int x) {
        constants$1941.const$0.set(seg.asSlice(index*sizeof()), x);
    }
    public static long sizeof() { return $LAYOUT().byteSize(); }
    public static MemorySegment allocate(SegmentAllocator allocator) { return allocator.allocate($LAYOUT()); }
    public static MemorySegment allocateArray(long len, SegmentAllocator allocator) {
        return allocator.allocate(MemoryLayout.sequenceLayout(len, $LAYOUT()));
    }
    public static MemorySegment ofAddress(MemorySegment addr, Arena arena) { return RuntimeHelper.asArray(addr, $LAYOUT(), 1, arena); }
}


