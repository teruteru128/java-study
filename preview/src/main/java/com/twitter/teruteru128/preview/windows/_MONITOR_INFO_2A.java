// Generated by jextract

package com.twitter.teruteru128.preview.windows;

import java.lang.invoke.MethodHandle;
import java.lang.invoke.VarHandle;
import java.nio.ByteOrder;
import java.lang.foreign.*;
import static java.lang.foreign.ValueLayout.*;
/**
 * {@snippet :
 * struct _MONITOR_INFO_2A {
 *     LPSTR pName;
 *     LPSTR pEnvironment;
 *     LPSTR pDLLName;
 * };
 * }
 */
public class _MONITOR_INFO_2A {

    public static MemoryLayout $LAYOUT() {
        return constants$2902.const$1;
    }
    public static VarHandle pName$VH() {
        return constants$2902.const$2;
    }
    /**
     * Getter for field:
     * {@snippet :
     * LPSTR pName;
     * }
     */
    public static MemorySegment pName$get(MemorySegment seg) {
        return (java.lang.foreign.MemorySegment)constants$2902.const$2.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * LPSTR pName;
     * }
     */
    public static void pName$set(MemorySegment seg, MemorySegment x) {
        constants$2902.const$2.set(seg, x);
    }
    public static MemorySegment pName$get(MemorySegment seg, long index) {
        return (java.lang.foreign.MemorySegment)constants$2902.const$2.get(seg.asSlice(index*sizeof()));
    }
    public static void pName$set(MemorySegment seg, long index, MemorySegment x) {
        constants$2902.const$2.set(seg.asSlice(index*sizeof()), x);
    }
    public static VarHandle pEnvironment$VH() {
        return constants$2902.const$3;
    }
    /**
     * Getter for field:
     * {@snippet :
     * LPSTR pEnvironment;
     * }
     */
    public static MemorySegment pEnvironment$get(MemorySegment seg) {
        return (java.lang.foreign.MemorySegment)constants$2902.const$3.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * LPSTR pEnvironment;
     * }
     */
    public static void pEnvironment$set(MemorySegment seg, MemorySegment x) {
        constants$2902.const$3.set(seg, x);
    }
    public static MemorySegment pEnvironment$get(MemorySegment seg, long index) {
        return (java.lang.foreign.MemorySegment)constants$2902.const$3.get(seg.asSlice(index*sizeof()));
    }
    public static void pEnvironment$set(MemorySegment seg, long index, MemorySegment x) {
        constants$2902.const$3.set(seg.asSlice(index*sizeof()), x);
    }
    public static VarHandle pDLLName$VH() {
        return constants$2902.const$4;
    }
    /**
     * Getter for field:
     * {@snippet :
     * LPSTR pDLLName;
     * }
     */
    public static MemorySegment pDLLName$get(MemorySegment seg) {
        return (java.lang.foreign.MemorySegment)constants$2902.const$4.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * LPSTR pDLLName;
     * }
     */
    public static void pDLLName$set(MemorySegment seg, MemorySegment x) {
        constants$2902.const$4.set(seg, x);
    }
    public static MemorySegment pDLLName$get(MemorySegment seg, long index) {
        return (java.lang.foreign.MemorySegment)constants$2902.const$4.get(seg.asSlice(index*sizeof()));
    }
    public static void pDLLName$set(MemorySegment seg, long index, MemorySegment x) {
        constants$2902.const$4.set(seg.asSlice(index*sizeof()), x);
    }
    public static long sizeof() { return $LAYOUT().byteSize(); }
    public static MemorySegment allocate(SegmentAllocator allocator) { return allocator.allocate($LAYOUT()); }
    public static MemorySegment allocateArray(long len, SegmentAllocator allocator) {
        return allocator.allocate(MemoryLayout.sequenceLayout(len, $LAYOUT()));
    }
    public static MemorySegment ofAddress(MemorySegment addr, Arena arena) { return RuntimeHelper.asArray(addr, $LAYOUT(), 1, arena); }
}


