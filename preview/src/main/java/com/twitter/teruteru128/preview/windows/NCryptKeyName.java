// Generated by jextract

package com.twitter.teruteru128.preview.windows;

import java.lang.invoke.MethodHandle;
import java.lang.invoke.VarHandle;
import java.nio.ByteOrder;
import java.lang.foreign.*;
import static java.lang.foreign.ValueLayout.*;
/**
 * {@snippet :
 * struct NCryptKeyName {
 *     LPWSTR pszName;
 *     LPWSTR pszAlgid;
 *     DWORD dwLegacyKeySpec;
 *     DWORD dwFlags;
 * };
 * }
 */
public class NCryptKeyName {

    public static MemoryLayout $LAYOUT() {
        return constants$1989.const$1;
    }
    public static VarHandle pszName$VH() {
        return constants$1989.const$2;
    }
    /**
     * Getter for field:
     * {@snippet :
     * LPWSTR pszName;
     * }
     */
    public static MemorySegment pszName$get(MemorySegment seg) {
        return (java.lang.foreign.MemorySegment)constants$1989.const$2.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * LPWSTR pszName;
     * }
     */
    public static void pszName$set(MemorySegment seg, MemorySegment x) {
        constants$1989.const$2.set(seg, x);
    }
    public static MemorySegment pszName$get(MemorySegment seg, long index) {
        return (java.lang.foreign.MemorySegment)constants$1989.const$2.get(seg.asSlice(index*sizeof()));
    }
    public static void pszName$set(MemorySegment seg, long index, MemorySegment x) {
        constants$1989.const$2.set(seg.asSlice(index*sizeof()), x);
    }
    public static VarHandle pszAlgid$VH() {
        return constants$1989.const$3;
    }
    /**
     * Getter for field:
     * {@snippet :
     * LPWSTR pszAlgid;
     * }
     */
    public static MemorySegment pszAlgid$get(MemorySegment seg) {
        return (java.lang.foreign.MemorySegment)constants$1989.const$3.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * LPWSTR pszAlgid;
     * }
     */
    public static void pszAlgid$set(MemorySegment seg, MemorySegment x) {
        constants$1989.const$3.set(seg, x);
    }
    public static MemorySegment pszAlgid$get(MemorySegment seg, long index) {
        return (java.lang.foreign.MemorySegment)constants$1989.const$3.get(seg.asSlice(index*sizeof()));
    }
    public static void pszAlgid$set(MemorySegment seg, long index, MemorySegment x) {
        constants$1989.const$3.set(seg.asSlice(index*sizeof()), x);
    }
    public static VarHandle dwLegacyKeySpec$VH() {
        return constants$1989.const$4;
    }
    /**
     * Getter for field:
     * {@snippet :
     * DWORD dwLegacyKeySpec;
     * }
     */
    public static int dwLegacyKeySpec$get(MemorySegment seg) {
        return (int)constants$1989.const$4.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * DWORD dwLegacyKeySpec;
     * }
     */
    public static void dwLegacyKeySpec$set(MemorySegment seg, int x) {
        constants$1989.const$4.set(seg, x);
    }
    public static int dwLegacyKeySpec$get(MemorySegment seg, long index) {
        return (int)constants$1989.const$4.get(seg.asSlice(index*sizeof()));
    }
    public static void dwLegacyKeySpec$set(MemorySegment seg, long index, int x) {
        constants$1989.const$4.set(seg.asSlice(index*sizeof()), x);
    }
    public static VarHandle dwFlags$VH() {
        return constants$1989.const$5;
    }
    /**
     * Getter for field:
     * {@snippet :
     * DWORD dwFlags;
     * }
     */
    public static int dwFlags$get(MemorySegment seg) {
        return (int)constants$1989.const$5.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * DWORD dwFlags;
     * }
     */
    public static void dwFlags$set(MemorySegment seg, int x) {
        constants$1989.const$5.set(seg, x);
    }
    public static int dwFlags$get(MemorySegment seg, long index) {
        return (int)constants$1989.const$5.get(seg.asSlice(index*sizeof()));
    }
    public static void dwFlags$set(MemorySegment seg, long index, int x) {
        constants$1989.const$5.set(seg.asSlice(index*sizeof()), x);
    }
    public static long sizeof() { return $LAYOUT().byteSize(); }
    public static MemorySegment allocate(SegmentAllocator allocator) { return allocator.allocate($LAYOUT()); }
    public static MemorySegment allocateArray(long len, SegmentAllocator allocator) {
        return allocator.allocate(MemoryLayout.sequenceLayout(len, $LAYOUT()));
    }
    public static MemorySegment ofAddress(MemorySegment addr, Arena arena) { return RuntimeHelper.asArray(addr, $LAYOUT(), 1, arena); }
}


