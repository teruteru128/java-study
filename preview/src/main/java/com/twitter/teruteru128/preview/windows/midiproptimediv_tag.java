// Generated by jextract

package com.twitter.teruteru128.preview.windows;

import java.lang.invoke.MethodHandle;
import java.lang.invoke.VarHandle;
import java.nio.ByteOrder;
import java.lang.foreign.*;
import static java.lang.foreign.ValueLayout.*;
/**
 * {@snippet :
 * struct midiproptimediv_tag {
 *     DWORD cbStruct;
 *     DWORD dwTimeDiv;
 * };
 * }
 */
public class midiproptimediv_tag {

    public static MemoryLayout $LAYOUT() {
        return constants$1642.const$4;
    }
    public static VarHandle cbStruct$VH() {
        return constants$1642.const$5;
    }
    /**
     * Getter for field:
     * {@snippet :
     * DWORD cbStruct;
     * }
     */
    public static int cbStruct$get(MemorySegment seg) {
        return (int)constants$1642.const$5.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * DWORD cbStruct;
     * }
     */
    public static void cbStruct$set(MemorySegment seg, int x) {
        constants$1642.const$5.set(seg, x);
    }
    public static int cbStruct$get(MemorySegment seg, long index) {
        return (int)constants$1642.const$5.get(seg.asSlice(index*sizeof()));
    }
    public static void cbStruct$set(MemorySegment seg, long index, int x) {
        constants$1642.const$5.set(seg.asSlice(index*sizeof()), x);
    }
    public static VarHandle dwTimeDiv$VH() {
        return constants$1643.const$0;
    }
    /**
     * Getter for field:
     * {@snippet :
     * DWORD dwTimeDiv;
     * }
     */
    public static int dwTimeDiv$get(MemorySegment seg) {
        return (int)constants$1643.const$0.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * DWORD dwTimeDiv;
     * }
     */
    public static void dwTimeDiv$set(MemorySegment seg, int x) {
        constants$1643.const$0.set(seg, x);
    }
    public static int dwTimeDiv$get(MemorySegment seg, long index) {
        return (int)constants$1643.const$0.get(seg.asSlice(index*sizeof()));
    }
    public static void dwTimeDiv$set(MemorySegment seg, long index, int x) {
        constants$1643.const$0.set(seg.asSlice(index*sizeof()), x);
    }
    public static long sizeof() { return $LAYOUT().byteSize(); }
    public static MemorySegment allocate(SegmentAllocator allocator) { return allocator.allocate($LAYOUT()); }
    public static MemorySegment allocateArray(long len, SegmentAllocator allocator) {
        return allocator.allocate(MemoryLayout.sequenceLayout(len, $LAYOUT()));
    }
    public static MemorySegment ofAddress(MemorySegment addr, Arena arena) { return RuntimeHelper.asArray(addr, $LAYOUT(), 1, arena); }
}


