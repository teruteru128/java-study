// Generated by jextract

package com.twitter.teruteru128.preview.windows;

import java.lang.invoke.MethodHandle;
import java.lang.invoke.VarHandle;
import java.nio.ByteOrder;
import java.lang.foreign.*;
import static java.lang.foreign.ValueLayout.*;
/**
 * {@snippet :
 * struct _CTL_INFO {
 *     DWORD dwVersion;
 *     CTL_USAGE SubjectUsage;
 *     CRYPT_DATA_BLOB ListIdentifier;
 *     CRYPT_INTEGER_BLOB SequenceNumber;
 *     FILETIME ThisUpdate;
 *     FILETIME NextUpdate;
 *     CRYPT_ALGORITHM_IDENTIFIER SubjectAlgorithm;
 *     DWORD cCTLEntry;
 *     PCTL_ENTRY rgCTLEntry;
 *     DWORD cExtension;
 *     PCERT_EXTENSION rgExtension;
 * };
 * }
 */
public class _CTL_INFO {

    public static MemoryLayout $LAYOUT() {
        return constants$2018.const$1;
    }
    public static VarHandle dwVersion$VH() {
        return constants$2018.const$2;
    }
    /**
     * Getter for field:
     * {@snippet :
     * DWORD dwVersion;
     * }
     */
    public static int dwVersion$get(MemorySegment seg) {
        return (int)constants$2018.const$2.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * DWORD dwVersion;
     * }
     */
    public static void dwVersion$set(MemorySegment seg, int x) {
        constants$2018.const$2.set(seg, x);
    }
    public static int dwVersion$get(MemorySegment seg, long index) {
        return (int)constants$2018.const$2.get(seg.asSlice(index*sizeof()));
    }
    public static void dwVersion$set(MemorySegment seg, long index, int x) {
        constants$2018.const$2.set(seg.asSlice(index*sizeof()), x);
    }
    public static MemorySegment SubjectUsage$slice(MemorySegment seg) {
        return seg.asSlice(8, 16);
    }
    public static MemorySegment ListIdentifier$slice(MemorySegment seg) {
        return seg.asSlice(24, 16);
    }
    public static MemorySegment SequenceNumber$slice(MemorySegment seg) {
        return seg.asSlice(40, 16);
    }
    public static MemorySegment ThisUpdate$slice(MemorySegment seg) {
        return seg.asSlice(56, 8);
    }
    public static MemorySegment NextUpdate$slice(MemorySegment seg) {
        return seg.asSlice(64, 8);
    }
    public static MemorySegment SubjectAlgorithm$slice(MemorySegment seg) {
        return seg.asSlice(72, 24);
    }
    public static VarHandle cCTLEntry$VH() {
        return constants$2018.const$3;
    }
    /**
     * Getter for field:
     * {@snippet :
     * DWORD cCTLEntry;
     * }
     */
    public static int cCTLEntry$get(MemorySegment seg) {
        return (int)constants$2018.const$3.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * DWORD cCTLEntry;
     * }
     */
    public static void cCTLEntry$set(MemorySegment seg, int x) {
        constants$2018.const$3.set(seg, x);
    }
    public static int cCTLEntry$get(MemorySegment seg, long index) {
        return (int)constants$2018.const$3.get(seg.asSlice(index*sizeof()));
    }
    public static void cCTLEntry$set(MemorySegment seg, long index, int x) {
        constants$2018.const$3.set(seg.asSlice(index*sizeof()), x);
    }
    public static VarHandle rgCTLEntry$VH() {
        return constants$2018.const$4;
    }
    /**
     * Getter for field:
     * {@snippet :
     * PCTL_ENTRY rgCTLEntry;
     * }
     */
    public static MemorySegment rgCTLEntry$get(MemorySegment seg) {
        return (java.lang.foreign.MemorySegment)constants$2018.const$4.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * PCTL_ENTRY rgCTLEntry;
     * }
     */
    public static void rgCTLEntry$set(MemorySegment seg, MemorySegment x) {
        constants$2018.const$4.set(seg, x);
    }
    public static MemorySegment rgCTLEntry$get(MemorySegment seg, long index) {
        return (java.lang.foreign.MemorySegment)constants$2018.const$4.get(seg.asSlice(index*sizeof()));
    }
    public static void rgCTLEntry$set(MemorySegment seg, long index, MemorySegment x) {
        constants$2018.const$4.set(seg.asSlice(index*sizeof()), x);
    }
    public static VarHandle cExtension$VH() {
        return constants$2018.const$5;
    }
    /**
     * Getter for field:
     * {@snippet :
     * DWORD cExtension;
     * }
     */
    public static int cExtension$get(MemorySegment seg) {
        return (int)constants$2018.const$5.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * DWORD cExtension;
     * }
     */
    public static void cExtension$set(MemorySegment seg, int x) {
        constants$2018.const$5.set(seg, x);
    }
    public static int cExtension$get(MemorySegment seg, long index) {
        return (int)constants$2018.const$5.get(seg.asSlice(index*sizeof()));
    }
    public static void cExtension$set(MemorySegment seg, long index, int x) {
        constants$2018.const$5.set(seg.asSlice(index*sizeof()), x);
    }
    public static VarHandle rgExtension$VH() {
        return constants$2019.const$0;
    }
    /**
     * Getter for field:
     * {@snippet :
     * PCERT_EXTENSION rgExtension;
     * }
     */
    public static MemorySegment rgExtension$get(MemorySegment seg) {
        return (java.lang.foreign.MemorySegment)constants$2019.const$0.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * PCERT_EXTENSION rgExtension;
     * }
     */
    public static void rgExtension$set(MemorySegment seg, MemorySegment x) {
        constants$2019.const$0.set(seg, x);
    }
    public static MemorySegment rgExtension$get(MemorySegment seg, long index) {
        return (java.lang.foreign.MemorySegment)constants$2019.const$0.get(seg.asSlice(index*sizeof()));
    }
    public static void rgExtension$set(MemorySegment seg, long index, MemorySegment x) {
        constants$2019.const$0.set(seg.asSlice(index*sizeof()), x);
    }
    public static long sizeof() { return $LAYOUT().byteSize(); }
    public static MemorySegment allocate(SegmentAllocator allocator) { return allocator.allocate($LAYOUT()); }
    public static MemorySegment allocateArray(long len, SegmentAllocator allocator) {
        return allocator.allocate(MemoryLayout.sequenceLayout(len, $LAYOUT()));
    }
    public static MemorySegment ofAddress(MemorySegment addr, Arena arena) { return RuntimeHelper.asArray(addr, $LAYOUT(), 1, arena); }
}


