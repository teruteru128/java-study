// Generated by jextract

package com.twitter.teruteru128.preview.windows;

import java.lang.invoke.MethodHandle;
import java.lang.invoke.VarHandle;
import java.nio.ByteOrder;
import java.lang.foreign.*;
import static java.lang.foreign.ValueLayout.*;
/**
 * {@snippet :
 * struct _EV_EXTRA_CERT_CHAIN_POLICY_PARA {
 *     DWORD cbSize;
 *     DWORD dwRootProgramQualifierFlags;
 * };
 * }
 */
public class _EV_EXTRA_CERT_CHAIN_POLICY_PARA {

    public static MemoryLayout $LAYOUT() {
        return constants$2224.const$0;
    }
    public static VarHandle cbSize$VH() {
        return constants$2224.const$1;
    }
    /**
     * Getter for field:
     * {@snippet :
     * DWORD cbSize;
     * }
     */
    public static int cbSize$get(MemorySegment seg) {
        return (int)constants$2224.const$1.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * DWORD cbSize;
     * }
     */
    public static void cbSize$set(MemorySegment seg, int x) {
        constants$2224.const$1.set(seg, x);
    }
    public static int cbSize$get(MemorySegment seg, long index) {
        return (int)constants$2224.const$1.get(seg.asSlice(index*sizeof()));
    }
    public static void cbSize$set(MemorySegment seg, long index, int x) {
        constants$2224.const$1.set(seg.asSlice(index*sizeof()), x);
    }
    public static VarHandle dwRootProgramQualifierFlags$VH() {
        return constants$2224.const$2;
    }
    /**
     * Getter for field:
     * {@snippet :
     * DWORD dwRootProgramQualifierFlags;
     * }
     */
    public static int dwRootProgramQualifierFlags$get(MemorySegment seg) {
        return (int)constants$2224.const$2.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * DWORD dwRootProgramQualifierFlags;
     * }
     */
    public static void dwRootProgramQualifierFlags$set(MemorySegment seg, int x) {
        constants$2224.const$2.set(seg, x);
    }
    public static int dwRootProgramQualifierFlags$get(MemorySegment seg, long index) {
        return (int)constants$2224.const$2.get(seg.asSlice(index*sizeof()));
    }
    public static void dwRootProgramQualifierFlags$set(MemorySegment seg, long index, int x) {
        constants$2224.const$2.set(seg.asSlice(index*sizeof()), x);
    }
    public static long sizeof() { return $LAYOUT().byteSize(); }
    public static MemorySegment allocate(SegmentAllocator allocator) { return allocator.allocate($LAYOUT()); }
    public static MemorySegment allocateArray(long len, SegmentAllocator allocator) {
        return allocator.allocate(MemoryLayout.sequenceLayout(len, $LAYOUT()));
    }
    public static MemorySegment ofAddress(MemorySegment addr, Arena arena) { return RuntimeHelper.asArray(addr, $LAYOUT(), 1, arena); }
}


