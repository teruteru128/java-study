// Generated by jextract

package com.twitter.teruteru128.preview.windows;

import java.lang.invoke.MethodHandle;
import java.lang.invoke.VarHandle;
import java.nio.ByteOrder;
import java.lang.foreign.*;
import static java.lang.foreign.ValueLayout.*;
/**
 * {@snippet :
 * struct _TRANSACTION_BASIC_INFORMATION {
 *     GUID TransactionId;
 *     DWORD State;
 *     DWORD Outcome;
 * };
 * }
 */
public class _TRANSACTION_BASIC_INFORMATION {

    public static MemoryLayout $LAYOUT() {
        return constants$526.const$1;
    }
    public static MemorySegment TransactionId$slice(MemorySegment seg) {
        return seg.asSlice(0, 16);
    }
    public static VarHandle State$VH() {
        return constants$526.const$2;
    }
    /**
     * Getter for field:
     * {@snippet :
     * DWORD State;
     * }
     */
    public static int State$get(MemorySegment seg) {
        return (int)constants$526.const$2.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * DWORD State;
     * }
     */
    public static void State$set(MemorySegment seg, int x) {
        constants$526.const$2.set(seg, x);
    }
    public static int State$get(MemorySegment seg, long index) {
        return (int)constants$526.const$2.get(seg.asSlice(index*sizeof()));
    }
    public static void State$set(MemorySegment seg, long index, int x) {
        constants$526.const$2.set(seg.asSlice(index*sizeof()), x);
    }
    public static VarHandle Outcome$VH() {
        return constants$526.const$3;
    }
    /**
     * Getter for field:
     * {@snippet :
     * DWORD Outcome;
     * }
     */
    public static int Outcome$get(MemorySegment seg) {
        return (int)constants$526.const$3.get(seg);
    }
    /**
     * Setter for field:
     * {@snippet :
     * DWORD Outcome;
     * }
     */
    public static void Outcome$set(MemorySegment seg, int x) {
        constants$526.const$3.set(seg, x);
    }
    public static int Outcome$get(MemorySegment seg, long index) {
        return (int)constants$526.const$3.get(seg.asSlice(index*sizeof()));
    }
    public static void Outcome$set(MemorySegment seg, long index, int x) {
        constants$526.const$3.set(seg.asSlice(index*sizeof()), x);
    }
    public static long sizeof() { return $LAYOUT().byteSize(); }
    public static MemorySegment allocate(SegmentAllocator allocator) { return allocator.allocate($LAYOUT()); }
    public static MemorySegment allocateArray(long len, SegmentAllocator allocator) {
        return allocator.allocate(MemoryLayout.sequenceLayout(len, $LAYOUT()));
    }
    public static MemorySegment ofAddress(MemorySegment addr, Arena arena) { return RuntimeHelper.asArray(addr, $LAYOUT(), 1, arena); }
}


